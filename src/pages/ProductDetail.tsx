...
export default function ProductDetail() {
  ...
  const [weight, setWeight] = useState(0.5); // Default 500g
  const [selectionMode, setSelectionMode] = useState<'quick' | 'custom'>('quick'); // 'quick' o 'custom'
  ...
  function handleAddToCart() {
    ...
    if (product.productType === 'weighted') {
      ...
      const weightText = weight < 1 ? `${weight * 1000}g` : `${weight}kg`;
      showToast(`${product.name} (${weightText}) agregado al carrito`, 'success');
    } else {
        ...
        <div>
          ...
          {product.productType === 'weighted' ? (
            <div className="mb-6">
              ...
              {/* Modo de selección: Rápida o Personalizada */}
              <div className="mb-4">
                <label className="block text-sm font-medium mb-2 text-text">Tipo de selección:</label>
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      setSelectionMode('quick');
                      setWeight(0.5); // Reset a 500g
                    }}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setWeight(Math.max(0.1, Math.min(10, val)));
                      }}
                      min={0.1}
                      max={10}
                      step={0.1}
                    ...
                    <button
                      onClick={() => setWeight(Math.min(10, weight + 0.1))}
                      className="w-10 h-10 flex items-center justify-center border-2 border-gray-300 rounded-lg hover:border-primary transition-colors font-bold text-lg"
                    >
                      +
                    </button>
              ...
              <div className="bg-bg-alt p-4 rounded-lg mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-primary">
                    ${((product.promoPrice || product.pricePerKg || 0) * weight).toLocaleString('es-AR')}
                  </span>
                </div>
                <p className="text-xs text-text-muted mt-1">
                  {weight < 1 ? `${weight * 1000}g` : `${weight}kg`} × ${(product.promoPrice || product.pricePerKg || 0).toLocaleString('es-AR')}/kg
                </p>
              </div>
            </div>
          ...
          >
            {product.productType === 'weighted' ? (
              <>Agregar {weight < 1 ? `${weight * 1000}g` : `${weight}kg`} al carrito</>
            ) : product.productType === 'combo' ? (
